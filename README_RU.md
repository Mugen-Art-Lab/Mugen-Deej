...
Legacy-контроллер отправляет строки такого вида:

```text
107|246|536|665|1020
```

Стандартная конфигурация ожидает пять значений от `0` до `1023` со скоростью `9600` бод. Extended-прошивка может дополнительно передавать кнопки; Mugen Deej автоматически определяет поддерживаемый режим контроллера.
...
- `src/setup/` — самостоятельная оболочка Setup и двуязычный интерфейс установщика.
