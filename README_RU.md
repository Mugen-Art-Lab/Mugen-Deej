...
Проект использует ту же базовую идею физического микшера на Arduino и принимает совместимые строки значений через Serial. Настольный клиент Mugen Deej, интерфейс, диагностика, логика подключения и входящая в комплект эталонная прошивка разрабатывались отдельно для этого проекта.
...
Mugen Deej превращает простой контроллер на Arduino с крутилками, фейдерами или другими аналоговыми регуляторами в понятный настольный микшер громкости Windows.
...
Готовую портативную сборку следует брать на странице **Releases**: распакуйте архив и запустите `MugenDeej.exe`.

Текущая проверенная сборка: **0.8.3**.
...
1. Прошейте `firmware/MugenDeej_Nano_RGB.ino` либо используйте совместимый скетч deej.
2. Подключите контроллер по USB.
3. Запустите `MugenDeej.exe` из архива релиза.
4. Выберите язык интерфейса.
5. Откройте **«Настроить регуляторы»** и назначьте каждому физическому регулятору нужную функцию.
...
- `firmware/` — эталонная прошивка Arduino Nano для пяти регуляторов и индикаторов WS2812.
