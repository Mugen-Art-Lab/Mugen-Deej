...
Integrated **#247** is the current UI-review candidate. It keeps the #244 slider/application-picker PASS, splits the Control layers intro deliberately, and aligns Virtual Xbox gamepad status wording with the adjacent Enable/Disable action (`disabled`, `enabling`, `enabled`, `enabled · waiting for controller`). CI/package is PASS; real-machine visual review is pending.
