...
Integrated **#242** is the current UI-review candidate. It keeps the #229 connection fix and #237 layered-encoder cleanup, adds deliberate two-line help in Button actions / layered encoder / layer notification, clarifies the application-profile picker, and polishes the slider/application picker layout. The normal slider settings no longer expose the technical config.json shortcut. CI/package is PASS; real-machine visual review is pending.
