...
$runtime = Replace-UiLiteral $runtime 'Для каждого слоя можно отдельно задать вращение и нажатие. «Как в основном слое» оставляет обычное назначение энкодера из выбранного профиля.' 'Для каждого слоя можно отдельно задать вращение и нажатие.'' + "`r`n" + ''«Как в основном слое» оставляет обычное назначение энкодера из выбранного профиля.' 'layered encoder balanced hint RU'
$runtime = Replace-UiLiteral $runtime 'Each layer can set rotation and push separately. “Same as base layer” keeps the normal encoder mapping from the selected profile.' 'Each layer can set rotation and push separately.'' + "`r`n" + ''“Same as base layer” keeps the normal encoder mapping from the selected profile.' 'layered encoder balanced hint EN'
$runtime = Replace-UiLiteral $runtime 'Показывает имя активного слоя при переключении T1/T2. Экран и позиция выбираются отдельно для многомониторной системы.' 'Показывает имя активного слоя при переключении T1/T2.'' + "`r`n" + ''Экран и позиция выбираются отдельно для многомониторной системы.' 'layer notification balanced hint RU'
$runtime = Replace-UiLiteral $runtime 'Shows the active layer name when T1/T2 changes. Choose a specific display and anchor for multi-monitor setups.' 'Shows the active layer name when T1/T2 changes.'' + "`r`n" + ''Choose a specific display and anchor for multi-monitor setups.' 'layer notification balanced hint EN'
