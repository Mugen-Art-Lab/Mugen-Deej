...
$runtime = Replace-UiLiteral $runtime '«Наследовать» = основное назначение; «Не использовать» = отключить в этом слое.' '«Как в основном слое» — оставить обычное действие; «Не использовать» — отключить кнопку только в этом слое.' 'layer editor hint line 1 RU'
$runtime = Replace-UiLiteral $runtime 'Inherit = base mapping; Do nothing = disable only in this layer.' '“Same as base layer” keeps the normal action; “Do nothing” disables the button only in this layer.' 'layer editor hint line 1 EN'
$runtime = Replace-UiLiteral $runtime 'Обычные действия — один раз при нажатии; геймпад Xbox — пока удерживается кнопка.' 'Обычные действия выполняются один раз при нажатии; виртуальный геймпад Xbox удерживает кнопку, пока вы держите кнопку на контроллере.' 'layer editor hint line 2 RU'
