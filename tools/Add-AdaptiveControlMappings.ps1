...
function Get-AdaptiveActionDisplay {
    ...
    $assignmentGrid.AutoSizeRowsMode = [System.Windows.Forms.DataGridViewAutoSizeRowsMode]::None
    ...
    $refreshAssignmentList = {
        ...
        for ($i = 0; $i -lt [int]$script:DetectedToggleCount; $i++) {
            ...
            foreach ($entry in @(
                @($(if ($script:Language -eq 'ru') { 'Включение' } else { 'Switch ON' }), [string]$pendingToggles[$i].on),
                @($(if ($script:Language -eq 'ru') { 'Выключение' } else { 'Switch OFF' }), [string]$pendingToggles[$i].off)
            )) {
                $totalCount++
                $action = ConvertTo-SafeAdaptiveAction -Action ([string]$entry[1])
        ...
        for ($i = 0; $i -lt [int]$script:DetectedEncoderCount; $i++) {
            ...
            $hasPush = (@($script:LatestEncoders).Count -gt $i -and [bool]$script:LatestEncoders[$i].HasPush)
            $entries = @(
                @($(if ($script:Language -eq 'ru') { 'По часовой' } else { 'Clockwise' }), [string]$pendingEncoders[$i].cw),
                @($(if ($script:Language -eq 'ru') { 'Против часовой' } else { 'Counter-clockwise' }), [string]$pendingEncoders[$i].ccw)
            )
            if ($hasPush) {
                $entries += ,@($(if ($script:Language -eq 'ru') { 'Нажатие' } else { 'Push' }), [string]$pendingEncoders[$i].push)
            }
