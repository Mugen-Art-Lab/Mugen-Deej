...
\r?\n        \) \{\r?\n            \$decoded = Decode-ButtonActionPayload -Payload \$Matches\[2\]\r?\n            \$valid = -not \[string\]::IsNullOrWhiteSpace\(\$decoded\)\r?\n        \}\r?\n\r?\n)(        if \(-not \$valid\) \{\r?\n            \$action = 'none'\r?\n        \})" `
    -Replacement @'
$1        # Preserve virtual gamepad actions during button normalization.
        if (
            -not $valid -and
            $script:VirtualGamepadFeatureAvailable -and
            (Test-MugenVirtualGamepadAction -Action $action)
